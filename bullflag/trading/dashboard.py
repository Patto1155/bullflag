...
<body>
    ...
    <form method="post" action="/clear_history" onsubmit="return confirm('Are you sure you want to delete all non-bullflag screenshots?');">
        <button type="submit" style="background:#e74c3c; color:white;">Clear History</button>
    </form>
    <hr>
...
def clear_history():
    import csv
    charts_dir = os.path.join(os.path.dirname(__file__), 'charts_live')
    bullflag_images = set()
    # Step 1: Parse CSV for bullflag images
    if os.path.exists(RESULTS_FILE):
        with open(RESULTS_FILE, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['prediction'] == 'bull_flag' and row['chart_image']:
                    bullflag_images.add(os.path.basename(row['chart_image']))
    # Step 2: List all files in charts_live/
    if os.path.exists(charts_dir):
        for fname in os.listdir(charts_dir):
            if fname not in bullflag_images:
                try:
